	...
	return (
		<motion.div
			...
			{isEditMode && (
				<button
					onClick={onDelete}
					className='absolute right-3 top-3 rounded-lg px-2 py-1.5 text-xs text-red-400 transition-colors hover:text-red-600'>
					删除
				</button>
			)}

			<div className='overflow-hidden rounded-xl border bg-white/40'>
				<img src={picture.image} alt={picture.description || picture.id} className='h-56 w-full object-contain bg-white' />
			</div>
